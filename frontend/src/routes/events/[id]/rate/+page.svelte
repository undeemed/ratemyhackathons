<script lang="ts">
	...
	const overallScore = $derived(() => {
		const rated = RATING_CATEGORIES.filter((c) => categoryRatings[c] > 0);
		if (rated.length === 0) return 0;
		return rated.reduce((sum, c) => sum + categoryRatings[c], 0) / rated.length;
	});
...
<svelte:head>
	<title>Rate {entity?.name ?? 'Event'} — RateMyHackathons</title>
</svelte:head>

<div class="mx-auto max-w-[800px] px-6 py-24">
	<a href="/events/{data.event?.id}" class="hover-line text-[10px] uppercase tracking-[0.3em] text-dim transition-colors hover:text-muted">&larr; Back</a>
	...
	{:else if success}
		<div class="mt-24 text-center" use:fadeIn>
			...
			<a href="/events/{entity.id}" class="mt-8 inline-block border border-text px-6 py-2 text-xs uppercase tracking-[0.2em] text-text transition-colors hover:bg-text hover:text-black">
				View Event
			</a>
		</div>
	{:else}
